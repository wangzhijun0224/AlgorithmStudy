...
/** Asserts that pointers *actual == *expected with length specified.
*/
#define CU_ASSERT_PTR_DATA_EQUAL(actual, expected, count) \
  { CU_assertImplementation(!(memcmp((const char*)(actual), (const char*)(expected), (size_t)(count))), __LINE__, ("CU_ASSERT_PTR_DATA_EQUAL(" #actual ","  #expected "," #count ")"), __FILE__, "", CU_FALSE); }
